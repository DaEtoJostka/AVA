...
class MinioClient:
    ...
    def __init__(self):
        self.client = Minio(
            ...
            access_key=cfg.minio.minio_credentials["access_key"],
            secret_key=cfg.minio.minio_credentials["secret_key"],
            secure=cfg.minio.MINIO_SECURE
        )
        logger.info(f"Initialized MinioClient with endpoint: {cfg.minio.minio_endpoint}")
    ...
    def put_file(self, bucket_name: str, object_name: str, file_content: Any, length=None|int) -> bool:
        try:
            if not self.client.bucket_exists(bucket_name):
